plot_benchmark_combined <- function(TP_matrix, TN_matrix, TP_input = NULL, TP_overall, TN_overall, plot_path, width = NULL, height = NULL){ # data_annot = NULL,
  ...
  method_order <- rownames(TP_matrix)

  ## TP-right annotation: TP overall performance----
  df_long <- TP_overall %>%
  ...
  # TN-right annotation: boxplot by seeds----
  pr_long <- TN_overall %>% select(method, seed, FPR)
  pr_long$method <- factor(pr_long$method, levels = rev(method_order))
  pr_long$method_numeric <- as.numeric(pr_long$method) # convert method to numeric positions(same as heatmap rows)
  pr_long$FPR_rev <- 1 - pr_long$FPR

  p_pr_TN <- ggplot(pr_long, aes(y = FPR_rev, x = method_numeric)) +  # Swap x and y x = fct_rev(factor(method_numeric))
    geom_violin(aes(fill = method), scale = "width", alpha = 0.4, color = NA) + # , position = position_identity()   side = "r"
    geom_half_boxplot(aes(fill = method), side = "r", outlier.shape = NA, outlier.color = NA, alpha = 1)+
    geom_point(aes(x = method_numeric - 0.2, color = method),  # Adjust position for flipped coords
               size = 1, alpha = 1) +
    geom_vline(xintercept = split_x, color = "grey", linewidth = 0.5) +
    geom_hline(yintercept = 0.95, linetype = "dashed", color = "red", linewidth = 0.5) +
    annotate("text", x = max(pr_long$method_numeric), y = 0.95, label = "FPR = 0.05", hjust = 1.5, vjust = 0, color = "red", size = 4)+
    scale_fill_manual(values = fill_col) +
    scale_color_manual(values = fill_col) +
    scale_x_continuous(
      breaks = 1:length(unique(pr_long$method)),
      limits = c(0.5, length(unique(pr_long$method))+0.5),
      expand = c(0, 0)
    ) +
    scale_y_continuous(
      expand = c(0.05,0),
  ...
  label_text <- "1 - False positive rate"

  TN_row_anno <- rowAnnotation(
    PR = anno_ggplot,
    annotation_label = label_text,
    annotation_name_side = "bottom",
    annotation_name_rot = 0,
  ...
  ## TP-column annotation for n_MGI, DEprop, and n_pathway ----
  ...
  TN_left_anno <- left_anno(TN_matrix)
  ...
  TP_args_list <- list(
    ...
    na_col = "grey90",
    column_title = paste0("Benchmark Results Across ", ncol(TP_matrix)," Cancer Types"),
  ...
  ## TN-main heatmap----
  TN_col_fun = circlize::colorRamp2(seq(0,1, length.out = 5), c("#EBF3FB", "#D7E7F7", "#B2D0ED", "#8BB3E3", "#5F8AC9"))
  # TN_max_pos <- apply(TN_matrix, 2, function(col){
  #   if (all(is.na(col)) || all(col == 0, na.rm = TRUE)) {
  #     return(integer(0))  # 返回空
  #   } else {
  #     dists <- abs(col - 0.05)
  #     min_dist <- min(dists, na.rm = TRUE)
  #     #if (col[which(dists == min_dist)] >0.01){
  #     if (any(col[which(dists == min_dist)] > 0.01)) {
  #       dists <- abs(round(col, 2) - 0.05)
  #       min_dist <- min(dists, na.rm = TRUE)
  #     }
  #   }
  #   return(which(dists == min_dist))}) #which(col == max(col, na.rm = TRUE)))

  TN_args_list <- list(
    matrix = TN_matrix, name = "FPR", col = TN_col_fun,
    na_col = "grey90",
    #column_title = paste0("Benchmark Results Across ", ncol(matrix)," Cancer Types"),
    ...
    row_names_side = "left",
    # row_split = annot_df$Type,
    left_annotation = TN_left_anno,
    right_annotation = TN_row_anno,
    # top_annotation = ha_top,
    # bottom_annotation = ha_bottom,
    row_dend_side = "left",
    border_gp = gpar(col = "black", lwd = 1.5),
    border = TRUE,
    rect_gp = gpar(col = "white", lwd = 1),
    width = ncol(TN_matrix) * unit(1, "cm"),
    height = nrow(TN_matrix) * unit(1, "cm"),
    cell_fun =  function(j, i, x, y, width, height, fill) {
      # grid.rect(x,y, width = width,height = height,
      #            gp = gpar(lwd = 3,col = "#ff6555",fill = fill))
      if (!is.na(TN_matrix[i, j])){
        if(TN_matrix[i, j] == 0){
          grid.text(sprintf("%.0f", TN_matrix[i, j]), x, y, gp = gpar(fontsize = 7.5))
        } else if (sprintf("%.2f", TN_matrix[i, j]) < 0.01){
          grid.text("< 0.01", x, y, gp = gpar(fontsize = 7.5))
        } else{
          grid.text(sprintf("%.2f", TN_matrix[i, j]), x, y, gp = gpar(fontsize = 7.5))
        }
      }
      # if (sum(i == as.integer(TN_max_pos[[j]])) > 0) {
      #   grid.rect(x, y, width, height, gp = gpar(col = "black", fill = NA, lwd = 2))
      # }
    },
  ...
  if (exists("ha_TN_bottom") && !is.null(ha_TN_bottom)) {
    TN_args_list$bottom_annotation <- ha_TN_bottom
  }

  TN_ht <- do.call(ComplexHeatmap::Heatmap, TN_args_list)
  ...
  # draw----
  ht <- TP_ht %v% TN_ht
  if (is.null(width)){width = ncol(TP_matrix) * 0.9}
  if (is.null(height)){height = nrow(TP_matrix)*1.1}
